...
To run amd64/nginx in read-only mode, you will need to mount a Docker volume to every location where amd64/nginx writes information. The default amd64/nginx configuration requires write access to `/var/cache` and `/var/run`. This can be easily accomplished by running amd64/nginx as follows:
